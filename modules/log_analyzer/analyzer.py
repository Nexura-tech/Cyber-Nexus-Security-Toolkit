...
def display_results(results):
    ...
    print(f"Failed login events: {results['failed_logins']}")
    print(f"Successful login events: {results['successful_logins']}")
    ...
    for ip, count in results["ip_counter"].most_common():
        print(f"{ip:<18} {count} occurrence(s)")
    ...
    if results["failed_logins"] > 0:
        print(
            f"[!] {results['failed_logins']} failed "
            "authentication event(s) detected."
        )
    else:
    ...
    if results["successful_logins"] > 0:
        print(
            f"[+] {results['successful_logins']} successful "
            "authentication event(s) detected."
        )
